-- =========================================================
-- ZETTERX CRM - MANUFACTURING SEED DATA (CONSOLIDATED SCHEMA)
-- Datos para negocio de vidrio templado y aluminio usando esquema actual
-- =========================================================

-- Configuración de organización (compatible con esquema actual)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM organization WHERE name = 'ZetterX Glass & Aluminum') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM users WHERE email = 'admin@zetterx.com') THEN
        INSERT INTO users (full_name, email, role) VALUES 
            ('Jonathan Admin', 'admin@zetterx.com', 'admin');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM users WHERE email = 'sales@zetterx.com') THEN
        INSERT INTO users (full_name, email, role) VALUES 
            ('Carlos Vendedor', 'sales@zetterx.com', 'sales');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM users WHERE email = 'production@zetterx.com') THEN
        INSERT INTO users (full_name, email, role) VALUES 
            ('María Producción', 'production@zetterx.com', 'ops');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM users WHERE email = 'installer@zetterx.com') THEN
        INSERT INTO users (full_name, email, role) VALUES 
...
DECLARE
    org_id UUID;
BEGIN
    SELECT id INTO org_id FROM organization WHERE name = 'ZetterX Glass & Aluminum';
    
    IF NOT EXISTS (SELECT 1 FROM emitters WHERE company_code = '155646463-86-001') THEN
        INSERT INTO emitters (
            organization_id, company_code, company_name, 
            ...
            org_id, '155646463-86-001', 'ZetterX Glass & Aluminum',
            '2', '155646463', '86', '001',
            'Parque Industrial, Zona Libre de Colón', 'COLON', 'COLON', 
            '507-1234-5678', 'info@zetterx.com',
...
BEGIN
    ...
    -- VENTANAS
    IF NOT EXISTS (SELECT 1 FROM products WHERE code = 'VEN-ALU-001') THEN
    ...
    -- PUERTAS
    IF NOT EXISTS (SELECT 1 FROM products WHERE code = 'PTA-ALU-001') THEN
        INSERT INTO products (emitter_id, code, description, price, cost, unit_code, itbms_rate, category) VALUES
    ...
    -- VIDRIOS Y PANELES
    IF NOT EXISTS (SELECT 1 FROM products WHERE code = 'VID-TEM-001') THEN
        INSERT INTO products (emitter_id, code, description, price, cost, unit_code, itbms_rate, category) VALUES
    ...
    IF NOT EXISTS (SELECT 1 FROM products WHERE code = 'ALU-PRF-001') THEN
        INSERT INTO products (emitter_id, code, description, price, cost, unit_code, itbms_rate, category) VALUES
    ...
    IF NOT EXISTS (SELECT 1 FROM products WHERE code = 'HER-COR-001') THEN
        INSERT INTO products (emitter_id, code, description, price, cost, unit_code, itbms_rate, category) VALUES
    ...
    -- SERVICIOS
    IF NOT EXISTS (SELECT 1 FROM products WHERE code = 'SRV-INS-001') THEN
        INSERT INTO products (emitter_id, code, description, price, cost, unit_code, itbms_rate, category) VALUES
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM customers WHERE name = 'Constructora Panamá S.A.') THEN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM installers WHERE full_name = 'Luis Méndez') THEN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM installer_teams WHERE name = 'Equipo Alpha Glass') THEN
        INSERT INTO installer_teams (emitter_id, name, lead_installer_id, max_concurrent_jobs, is_active) VALUES
            (emitter_id, 'Equipo Alpha Glass', luis_id, 3, true),
            (emitter_id, 'Equipo Beta Doors', ana_id, 2, true);
    END IF;
    
    -- Miembros de equipos
    IF NOT EXISTS (SELECT 1 FROM installer_team_members WHERE installer_id = luis_id) THEN
        INSERT INTO installer_team_members (team_id, installer_id, is_lead) 
        SELECT t.id, luis_id, true FROM installer_teams t WHERE t.name = 'Equipo Alpha Glass'
        UNION ALL
        SELECT t.id, carlos_id, false FROM installer_teams t WHERE t.name = 'Equipo Alpha Glass'
        UNION ALL
        SELECT t.id, ana_id, true FROM installer_teams t WHERE t.name = 'Equipo Beta Doors'
        UNION ALL
        SELECT t.id, miguel_id, false FROM installer_teams t WHERE t.name = 'Equipo Beta Doors';
    END IF;
END $$;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM price_zones WHERE name = 'Panamá Centro') THEN
...
-- Work Orders de ejemplo para el negocio
DO $$
DECLARE
    emitter_id UUID;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM work_orders WHERE title LIKE '%Edificio Residencial%') THEN
        INSERT INTO work_orders (
            emitter_id, customer_id, title, status, estimated_value,
            address_line, city, province, notes, created_by, assigned_to
        ) VALUES 
            (emitter_id, constructora_id, 'Ventanas Edificio Residencial - Torre A', 'quoted', 12500.00,
             'Costa del Este, Proyecto Residencial Vista Mar', 'PANAMA', 'PANAMA',
             '24 ventanas corredizas para apartamentos. Vidrio templado 6mm. Instalación por plantas.',
             sales_user_id, sales_user_id),
            
            (emitter_id, juan_carlos_id, 'Renovación Puertas y Ventanas Casa', 'approved', 3200.00,
             'Altos de Panamá, Casa 15A', 'PANAMA', 'PANAMA',
             '3 ventanas corredizas sala/comedor + 1 puerta corrediza terraza. Incluye mosquiteros.',
             sales_user_id, sales_user_id);
    END IF;
END $$;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM weekly_targets WHERE year = current_year) THEN
        INSERT INTO weekly_targets (emitter_id, year, week_number, target_amount, target_orders) VALUES
            ...
            (emitter_id, current_year, 1, 3500.00, 2),
            (emitter_id, current_year, 2, 4200.00, 3),
            (emitter_id, current_year, 3, 4800.00, 3),
            (emitter_id, current_year, 4, 5200.00, 4),
            -- Segundo trimestre (pico de temporada seca)
            (emitter_id, current_year, 13, 6500.00, 5),
            (emitter_id, current_year, 14, 7200.00, 5),
            (emitter_id, current_year, 15, 7800.00, 6),
            (emitter_id, current_year, 16, 8000.00, 6),
            -- Temporada de lluvia (menor actividad)
            (emitter_id, current_year, 26, 4500.00, 3),
            (emitter_id, current_year, 27, 4200.00, 3);
    END IF;
END $$;
