...
interface AdvancedConfirmationModalProps {
  ...
  productName: string
  relations: { invoiceItems: number; quoteItems: number }
  loading?: boolean
}
...
export function AdvancedConfirmationModal({
  ...
  const hasRelations = relations.invoiceItems > 0 || relations.quoteItems > 0
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md animate-in slide-in-from-bottom-4 duration-300 border border-slate-200/60">
        ...
        <div className="p-6">
          ...
          {hasRelations && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-4">
              ...
              <div className="space-y-2 text-sm text-yellow-700">
                {relations.invoiceItems > 0 && (
                  <div className="flex items-center">
                    <FileText className="w-4 h-4 mr-2" />
                    <span>{relations.invoiceItems} elementos en facturas</span>
                  </div>
                )}
                {relations.quoteItems > 0 && (
                  <div className="flex items-center">
                    <ShoppingCart className="w-4 h-4 mr-2" />
                    <span>{relations.quoteItems} elementos en cotizaciones</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <p className="text-gray-600 text-sm">
            {hasRelations 
              ? 'Puedes eliminar solo el producto o eliminarlo junto con todos sus registros asociados.'
          ...
          {hasRelations && (
            ...
            >
              {loading ? (
                ...
                'Eliminar todo (producto + registros)'
              )}
            </Button>
          )}
